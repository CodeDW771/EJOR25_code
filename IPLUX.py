...
def alg(envpara, config):
    """Run comparison algorithm IPLUX, paper (Distributed Optimization With Coupling Constraints)"""
    begin = time.time()
    ...
    for i in range(N):
        Sigma_b.append(coo_matrix( (ATA2[i]+HTH[i])/rho_0 + alpha_0 * np.eye((N*M*K*I)))) 
    ...
    for i in range(N):
        ...
        Sigma_tensor_p[i][M*K*I : M*K*I+M*K , M*K*I : M*K*I+M*K] = (GTG[i].toarray())/rho_0+(1/(N*N*rho_0)+alpha_0)*np.eye((M*K))
    ...
    y = [[] for i in range(N)]
    ax_z = [[] for i in range(N)]
    ax_u = [[] for i in range(N)]
    ...
    for i in range(N):
        ...
        y[i].append(np.zeros(N*M*K*I+M*K))
        ax_z[i].append(np.zeros(M*K+N*N*M*K*I+N*M*K)) 
        ax_u[i].append(np.zeros(M*K+N*N*M*K*I+N*M*K)) 
    ...
    while stopcon(t):
        ...
        for i in range(N):
            temp = 0
            for j in range(N):
                temp = temp + PW[i][j] * ax_u[j][0]
                    
            eta.append(-(ax_z[i][0]+Dp[i])/rho_0 + temp )
                  
        for i in range(N):
            
            Gamma.append( Psi[i] + Phi[i].T @ eta[i] + Sigma_0[i] @ y[i][0] - alpha_0 * y[i][0]  )
            ...
            y[i].append(np.concatenate((x_minus[i][1][:i*M*K*I],x[i][1],x_minus[i][1][i*M*K*I:],w[i][1])))
            
            ax_u[i].append( (1/rho_0) * Phi[i] @ y[i][0] + eta[i] )
            
        for i in range(N):
            temp = 0
            for j in range(N):
                temp = temp + PH[i][j] * ax_u[j][1]
                    
            ax_z[i].append( ax_z[i][0] + rho_0 * temp )
        ...
        ax_z = [sublist[1:] for sublist in ax_z]
        ax_u = [sublist[1:] for sublist in ax_u]
    ...
    plt.title( f"Value of penalty factor: rho= {rho_0}, alpha= {alpha_0}")
    # plt.xlim(0,5001)
    # plt.ylim(0,6800000)
