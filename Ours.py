...
def alg(envpara, config):
    ...
    # envpara.edgecnt: the number of edges in traffic network,  E
    suminv = np.zeros(envpara.edgecnt)  
    Qtuda = np.zeros((N,envpara.edgecnt))  # Qtuda
    for i in range(N):
        Qtuda[i] = Q[i] @ np.ones(M*K*I)
        suminv = suminv + Qtuda[i]     #  suminv: sum_i Qtuda_i
        #Q[i] = coo_matrix(Q[i])
    for i in range(envpara.edgecnt):
        if suminv[i] > 1e-5:
            suminv[i] = 1.0 / suminv[i]
    suminv = np.diag(suminv)   #Construct an anti-diagonal matrix whose diagonal elements are given by vector sum_i Qtuda_i
    ...
    for i in range(N):
        Qtudatuda.append(coo_matrix(np.diag(suminv @ Qtuda[i])))   # Qtudatuda_i matrix and sparsity
        H.append(coo_matrix(np.kron(envpara.Lap[i], np.identity(N*M*K*I)).T))   # H_i matrix and sparsity
        Atuda[i][:,i*M*K*I:(i+1)*M*K*I] = A.toarray()  # Atuda_i
        Atuda_s.append(coo_matrix(Atuda[i]))
        Phi.append(vstack((Atuda_s[i],H[i]))) # Phi_i matrix and sparsity
        Dp.append(D[i])  
    ...
    Theta = coo_matrix(Theta)  # Theta=[Q_1,Q_2,...,Q_N]
    ...
    abs_con_viopre = [0 for i in range(maxT+1)]
    abs_con_vioaf = [0 for i in range(maxT+1)]
    abs_con_viod = [0 for i in range(maxT+1)]
    abs_con_violamb = [0 for i in range(maxT+1)]
    ...
    while stopcon(t):
        ...
        for i in range(N):
            ...
            d[i].append(delta[i] + A @ (x[i][1] - x[i][0]))
       ...
       # perf_alg2[t+1] = funcg()
        abs_con_violamb[t+1] = np.linalg.norm(sum(x[0][1] - x[0][0]))
       ...
       # objective function value
        ...
        abs_con_viopre[t+1] = np.linalg.norm(tempsum[:M*K])
        abs_con_vioaf[t+1] = np.linalg.norm(tempsum[M*K:])
        abs_con_viod[t+1] = np.linalg.norm(sum(d[0][0]))
    ...
    #np.save(config["savedir"] + "conpre.npy", abs_con_viopre)
    #np.save(config["savedir"] + "conaf.npy", abs_con_vioaf)
    #np.save(config["savedir"] + "cond.npy", abs_con_viod)
    #np.save(config["savedir"] + "conlamb.npy", abs_con_violamb)
